		...
		try {
			...
			for (int i = 1; i < valuesCSV.size(); i++) {
				...
				if (strList.size() >= 20) {
					...
					double[] db = { Double.parseDouble(latitude),
							Double.parseDouble(longitude) };
					DoublePoint x = new DoublePoint(db);
					arrPoint.add(x);
					// List of All crimes
					crimeEntryList.add(new CrimeEntry(strList.get(2), strList
							.get(3), strList.get(5), strList.get(6), strList
							.get(7), x, strList.get(17)));
				}
			...
			DBSCANClusterer2<Clusterable> dbScanCLusterable = new DBSCANClusterer2<>(
					2, 0, new HonaluluMeasure());

			java.util.List<Cluster<Clusterable>> adf = dbScanCLusterable
					.cluster(crimeEntryList);
			System.out.println("Size " + adf.size());
			stpWatch.stop();

			System.out.println("Time for clustering "
					+ stpWatch.getElapsedSeconds());
			...
			for (int i = 0; i < adf.size(); i++) {
				...
				if (pointList.size() == 1) {
					CrimeEntry currPoint = (CrimeEntry) pointList.get(0);
					writer.write((1) + ". " + currPoint.toString()
							+ "\n");
					double[] pointCoordinates = currPoint.point.getPoint();

					centroidList.put(new DoublePoint(pointCoordinates), "");

					writer.write("Cluster Centroid Point  :"
							+ pointCoordinates[0] + "," + pointCoordinates[1]
							+ "\n");
					writer.write("*******************************\n");

					System.out.println("Number of Point in Cluster : "
							+ currCluster.getPoints().size());
				} else {
					...
					double[] db = { centralLatitude * 180 / Math.PI,
							centralLongitude * 180 / Math.PI };
					centroidList.put(new DoublePoint(db), "");

					writer.write("Cluster Centroid Point  :" + db[0] + ","
							+ db[1] + "\n");
					writer.write("*******************************\n");
